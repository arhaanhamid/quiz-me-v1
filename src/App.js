    ...
    async function getResponse(){
        ...
        setQuizData(() => {
            ...
            allOptions.map((option, index) => {
                ...
                for(let i=0; i<4; i++){
                    Math.random() > 0.5 ?
                    tempAnswers.push({option: option[i], isHeld: false, id: nanoid()}) :
                    tempAnswers.unshift({option: option[i], isHeld: false, id: nanoid()});
                }
        ...
        <Router>
            <main>
                <Routes>
                    ...
                    <Route path="/quizpage" element={<QuizPage quizData={quizData} optionSelected={optionSelected} />} />
                </Routes>
            </main>
        </Router>
    )
